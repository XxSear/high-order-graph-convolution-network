...
def train(model, data):
    ...
    out = model(data.node_feature, data.edges_lists)
...
def test(model, data):
    model.eval()
    pred = model(data.node_feature, data.edges_lists).max(1)[1]
    # print(pred)
...
if __name__ == '__main__':
    ...
    print(data.edges_lists[0].dtype)
    print(data.edges_lists[1].size())
    print(data.edges_lists[0].size())

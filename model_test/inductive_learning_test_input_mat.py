...
class InductiveLearningTestMat(object):
    ...
    def get_adj_mat(self, device='cpu'):
        print('bulit adj mat')
        self.train_adj_mat = [edge_list_2_adj_mat(graph.edge_list, node_num=graph.node_num, device=device) for graph in self.dataset.train_graphs]
        self.valid_adj_mat = [edge_list_2_adj_mat(graph.edge_list, node_num=graph.node_num, device=device) for graph in self.dataset.valid_graphs]
        self.test_adj_mat = [edge_list_2_adj_mat(graph.edge_list, node_num=graph.node_num, device=device) for graph in self.dataset.test_graphs]
    ...
    def train(self):
        ...
        for _ in range(len(self.dataset.train_graphs)):
            ...
            for adj_mat in self.train_adj_mat[idx]:
    ...
    def start(self, display=True):
        ...
        for epoch in range(1, self.epochs+1):
            ...
            test_f1, test_p, test_r = self.metrics_model(self.dataset.test_graphs, self.test_adj_mat)
...
if __name__ == '__main__':
    ...
    # from GNN_Implement.model.IGCN import IGCN
    from GNN_Implement.model.modified_model.ppi_IGCN import IGCN
    import torch

    torch.cuda.set_device(1)
    demo = InductiveLearningTestMat(IGCN, PPI())
    demo.start()